...
根构建依次执行 `build:lib:host`、`build:lib:client` 与 `build:web`。Host lib 阶段先以 bootstrap 方式运行 `tsc -b tsconfig.host.json`，再运行 `tsdown --env.DSH_BUILD_FACE host`，随后严格重跑 Host tsc。首次编译即使 Client 依赖在生成前访问 `/remote` 声明也会发射 Host JavaScript；Host tsdown 生成这些声明，重跑必须成功。Typert generator 由正常 Host Project Reference 图编译，并在这次 tsdown 中以 Host aggregate 为唯一 `ts.Program` 种子运行。Client lib 阶段随后运行 `tsc -b tsconfig.client.json` 与 `tsdown --env.DSH_BUILD_FACE client`，使用刚生成的 Remote Client 声明和运行时贡献，但不再次启动 Typert。
