...
Typert 只在 Host tsdown 中以 `tsconfig.host.json` 为种子运行。首次 Host tsc 会发射 Host JavaScript，但可能报告经由 Client 依赖进入的 `/remote` 声明尚未解析；Host tsdown 生成这些声明，第二次 Host tsc 必须成功，Client tsc 才能开始。Client tsdown 不启动 Typert。`pnpm run typecheck` 因此先执行完整 Host lib 阶段，再运行 Client tsc；`pnpm run build` 继续执行 Client tsdown 和 Web 构建。该顺序的决策记录见 [API Remotes 生成约定构建 Note](../.agents/notes/implemented/process/2026-08-08-api-remotes-generated-contract-build.md)。
