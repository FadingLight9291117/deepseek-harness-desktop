...
The root build runs `build:lib:host`, `build:lib:client`, and `build:web` in order. The Host lib phase makes a bootstrap `tsc -b tsconfig.host.json` pass, then runs `tsdown --env.DSH_BUILD_FACE host`, and repeats Host tsc strictly. The bootstrap emits Host JavaScript even when a Client dependency reaches generated `/remote` declarations before they exist; Host tsdown generates them, and the repeated pass must succeed. The normal Host Project Reference graph compiles the Typert generator, which runs during this tsdown pass with the Host aggregate as its only `ts.Program` seed. The Client lib phase then runs `tsc -b tsconfig.client.json` and `tsdown --env.DSH_BUILD_FACE client`, consuming the newly generated Remote Client declarations and runtime contributions without starting Typert again.
