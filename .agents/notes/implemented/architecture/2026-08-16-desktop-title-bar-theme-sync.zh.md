...
桌面应用的原生窗口标题栏只跟随操作系统外观，而应用自身带有独立的浅色/深色/系统主题偏好（设置"常规"页的外观行，持久化在 `$DSH_HOME/settings.yaml` 的 `ui-theme.preference`）。当应用设为深色而系统为浅色（或相反）时，标题栏与内容错配。此前的沉浸式标题栏尝试（自绘拖拽条，[2026-08-16-immersive-title-bar](2026-08-16-immersive-title-bar.md)，随后被回退）因难看被否决；保留系统原生标题栏，让它跟随应用自身主题。
