...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      // The desktop profile resolves its Loader rows through package runtime
      // entries, so its assembled prompt snapshot belongs to the built lane.
      ...(process.env.DSH_EXAMPLE_MODE === 'lib' ? ['apps/desktop/tests/**/*.snapshot.ts'] : []),
